page 50050 "Barcode Label Printing"
{
    ...
    begin
        ...
        if TempItemLabel.FindFirst then
            rec := TempItemLabel;
    end;
    ...
    begin
        if Rec.FindSet then
            repeat
                if Rec."Label Code" <> '' then begin
                    TempLabelCommand."Label Code" := Rec."Label Code";
                    if TempLabelCommand.Insert then;
                    if ItemLabel.Get(Rec."Store No.", Rec."Item No.", Rec.Variant, Rec."Unit of Measure", Rec."Label Is Valid on Date", Rec."Label Code") then begin
                        ItemLabel.TransferFields(Rec);
                        ItemLabel.Printed := false;
                        ItemLabel."Order From Code" := 'I' + Rec."Item No.";
        ...
        if TempLabelCommand.FindSet then
            repeat
                ItemLabel.SetRange("Store No.", Rec."Store No.");
                ItemLabel.SetRange("Label Code", TempLabelCommand."Label Code");
                ItemLabel.SetRange("Label Is Valid on Date", Rec."Label Is Valid on Date");
                ItemLabel.SetRange(Printed, false);
                ItemLabel.SetRange("Order From Code", 'I' + Rec."Item No.");
